...
def cornersHeuristic(state, problem):
    ...
    # 注意：这里的state和之前问题中的state定义不同
    node = state[0] #当前位置x,y
    Visited_Corners = state[1] #在isGoalState中定义：当到达角落，若该角位置坐标不在Visited里，则把它放入
    unVisited_Corners = []
    h_sum = 0
    ...
    while len(unVisited_Corners) > 0:
        distance = []
        for corner in unVisited_Corners: #每次取出一个角落点
            distance.append(util.manhattanDistance(node, corner)) #当前位置和角落点的曼哈顿距离
        h_sum += min(distance)
        node = corner #令当前点为角落点，计算到下一个点的曼哈顿距离
        unVisited_Corners.remove(corner)
    return h_sum # 返回到所有角落最短的曼哈顿距离*之和*
