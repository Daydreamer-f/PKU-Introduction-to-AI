...
def depthFirstSearch(problem):
    ...
    #print 'Start',proble.getSatrtState()
...
def breadthFirstSearch(problem):
     ...
     Frontier = util.Queue()
     Visited = []
     Frontier.push( (problem.getStartState(), []) ) #item为一个元组：第一个元素为state，int型（记录吃豆人的当前位置坐标）；第二个元素是actions，是一个列表（记录走的每一步）
     #print 'Start',problem.getStartState()
     Visited.append( problem.getStartState() )
    ...
    while Frontier.isEmpty() == 0:
        ...
        if state not in Visited:
            ...
            for next in problem.getSuccessors(state):
                n_state = next[0]
                n_direction = next[1]
                n_cost = next[2] #next有三个变量，但是n_cost用不上
                if n_state not in Visited:
    ...
    while Frontier.isEmpty() == 0:
        ...
        if state not in Visited:
            ...
            for next in problem.getSuccessors(state):
                ...
                n_cost = next[2] #next有三个变量，但是n_cost用不上
                if n_state not in Visited:
                    new_actions = actions + [n_direction]
                    new_costs = problem.getCostOfActions(new_actions) + heuristic(n_state,problem)
                    Frontier.update( (n_state,new_actions), new_costs )
